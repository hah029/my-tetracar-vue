...
export const DEFAULT_LANES = [-3, -1, 1, 3];
...
export function calculateRoadWidth(lanes: number[], edgeOffset: number): number {
...
export function getEdgePositions(lanes: number[], edgeOffset: number): { left: number; right: number } {
  const minLane = Math.min(...lanes);
  const maxLane = Math.max(...lanes);
  return {
