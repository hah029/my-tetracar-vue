<template>
    <div class="controls_global_container" @click="changeControlType()">
        ...
        <TransitionGroup v-if="controlsType == 'desktop'" name="buttons_group_showing" tag="div"
            ...
            <div v-for="(row, index) in availableControls" v-if="rowView" class="settings_row addit_font" :key="row.id"
                :style="{ animationDelay: `${index * 0.06}s` }">
                <span>{{ row.text }}</span>
                <div class="right_part">
                    <div v-if="row.id == 1 || row.id == 2" class="button_icon">
                        <img class="vector_img img_correction" :class="{ 'arrow--right': row.id == 2 }"
                            src="@/assets/images/arrow.svg" />
                    </div>
                    <div class="button_icon" :class="{ 'button_icon--extended': row.id == 3 || row.id == 4 }">{{ row.key }}
                    </div>
                </div>
            </div>
        </TransitionGroup>
