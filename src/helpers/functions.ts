...
export function makeWeightedChoice(mapper: object): string {
  const dropTypes = Object.keys(mapper) as (keyof typeof mapper)[];
  const choices: (keyof typeof mapper)[] = [...dropTypes];

  let totalWeight = 0;
  const cumulativeWeights = Object.values(mapper).map(
    (w) => (totalWeight += w),
  );
